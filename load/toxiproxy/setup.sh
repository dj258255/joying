...
#
# 앱은 프록시 포트를 보고 뜬다.
#   MONGODB_PORT=27019 REDIS_PORT=16381 ./gradlew bootRun
#
# 사용:
#   load/toxiproxy/setup.sh up          프록시를 만든다
#   load/toxiproxy/setup.sh latency 300 몽고 응답을 300ms 늦춘다
#   load/toxiproxy/setup.sh reset 0.3   몽고 연결을 30% 확률로 끊는다
...
case "${1:-}" in
  up)
    ...
    NET=$(docker inspect joying-mongodb --format '{{range $k,$v := .NetworkSettings.Networks}}{{$k}}{{end}}')
    docker rm -f "$NAME" >/dev/null 2>&1 || true
    docker run -d --name "$NAME" --network "$NET" \
      -p 8475:8474 -p 27019:27019 -p 16381:16381 \
      ghcr.io/shopify/toxiproxy:2.11.0 >/dev/null
    sleep 3
    curl -sf -X POST "$API/proxies" \
      -d '{"name":"mongodb","listen":"0.0.0.0:27019","upstream":"joying-mongodb:27017","enabled":true}' >/dev/null
    curl -sf -X POST "$API/proxies" \
      -d '{"name":"redis","listen":"0.0.0.0:16381","upstream":"joying-redis:6379","enabled":true}' >/dev/null
    echo "프록시 준비됨. MONGODB_PORT=27019 REDIS_PORT=16381 로 앱을 띄워라"
    ;;

  latency)
    MS=${2:-300}
    curl -sf -X POST "$API/proxies/mongodb/toxics" \
      -d "{\"name\":\"mongo_latency\",\"type\":\"latency\",\"stream\":\"downstream\",\"attributes\":{\"latency\":$MS,\"jitter\":100}}" >/dev/null
    echo "몽고 응답에 ${MS}ms(±100) 지연을 넣었다"
    ;;

  reset)
    RATE=${2:-0.3}
    curl -sf -X POST "$API/proxies/mongodb/toxics" \
      -d "{\"name\":\"mongo_reset\",\"type\":\"reset_peer\",\"stream\":\"downstream\",\"toxicity\":$RATE,\"attributes\":{\"timeout\":0}}" >/dev/null
    echo "몽고 연결을 ${RATE} 확률로 끊는다"
    ;;
