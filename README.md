...
| 탭을 하나 더 열면 채팅이 엉뚱한 서버로 붙었다 | 방 500쌍을 앞단에 보내 어디로 가는지 셌다 | 63.3% → **0%** |
| 배포가 한 번도 성공할 수 없는 상태였다 | 서버 없이 이 기계를 대상으로 끝까지 돌렸다 | 여섯 군데를 고쳐 **응답 200** |
...
- **고치는 길에 배운 것도 적었다.** `@Retryable` 과 `@Transactional` 의 순서, `location ^~`
  가 정규식 자리를 막는 것, `map` 이 `rewrite` 뒤에 계산되는 것. 셋 다 모르고 지나갔으면
  잘못된 수치를 적거나 고치지 못했을 것이다.
