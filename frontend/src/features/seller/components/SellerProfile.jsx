...
const SellerProfile = ({ seller = {}, sellerId, compact = false }) => {
  ...
  return (
    <div className={compact ? "p-3 space-y-2" : "p-4 md:p-6 space-y-4 md:space-y-6"}>
      {!compact && <h3 className="text-lg md:text-xl font-bold text-gray-900">판매자 정보</h3>}

      {/* 판매자 기본 정보 */}
      <div 
        className={compact ? "flex items-center space-x-3 cursor-pointer" : "flex items-center space-x-3 md:space-x-4 cursor-pointer hover:opacity-80 transition-opacity"}
      ...
      >
        ...
        />
        <div>
          <div className={compact ? "font-semibold text-base text-gray-900 flex items-center gap-2" : "font-semibold text-base md:text-lg text-gray-900 flex items-center gap-2"}>
            {nickname}
