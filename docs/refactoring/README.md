...
- **재접속 복구가 아직 연결되지 않았다.** 끊긴 사이에 놓친 것을 받는 조회가 만들어져 있는데 화면이 부르지 않는다. `어디서 시작했는가`에 적은 넷 중 마지막 하나다.
- **메시지 순서를 서버가 보장하지 않는다.** 시퀀스가 없어 브라우저가 시각으로 정렬해 덮고 있다.
- **같은 메시지를 두 번 보내면 두 건이 된다.** 멱등 열쇠가 없다.
