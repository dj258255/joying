...
먼저 원인을 고정한다. 토스를 부르는 그 순간에 트랜잭션이 열려 있는지와 나가 있는 커넥션 수를 잰다.
...
### 풀이 비면 상관없는 조회가 2초를 기다린다

커넥션을 쥐고 있으면 풀이 빈다는 것은 따라 나오는 결과지만, 실제로 몇 초를 기다리는지는 따로 재야 안다.

커넥션 5개에 승인 5건을 동시에 밀어 넣고, 그 다섯이 모두 토스 자리에 들어간 뒤에 **결제와 아무 상관 없는 조회** 하나를 던져 커넥션을 받기까지 걸린 시간을 쟀다. 운영에서 이 자리는 채팅 목록이나 상품 조회다.

| | 상관없는 조회가 기다린 시간 (3회) |
|---|---|
| 옛 모양 | 2,021ms · 2,027ms · 2,035ms |
| 새 모양 | 2ms · 2ms · 1ms |

**토스가 걸린 시간이 그대로 남의 대기 시간이 된다.** 옛 모양에서 기다린 2초는 토스에 머문 2초와 같다. 승인과 아무 상관 없는 조회가 승인이 끝날 때까지 서 있었다.

조건: 커넥션 5개, 동시 승인 5건, 토스 2초. 운영값은 커넥션 30개에 응답 제한 10초다. **비율을 맞춘 것이 아니라 풀이 확실히 비는 조건을 만든 것이다.** 절대 시간이 아니라 기다리느냐 아니냐를 본다. 운영에서는 30건이 겹쳐야 같은 일이 나고, 그때 기다리는 시간은 2초가 아니라 토스가 걸린 만큼이다.
...
## 상황 25. 골라 놓고 근거를 재지 않은 것

**앞단이 `hash $chat_room_key consistent` 를 쓴다.** `consistent` 를 붙인 이유는 노드가 늘거나 줄 때 붙어 있던 연결이 한꺼번에 끊기는 것을 막으려는 것이라고 적어 두었다. **그런데 그것을 재지 않았다.** 골라 놓고 근거는 문장으로만 있었다.

### 백엔드를 띄우지 않고 쟀다

방이 어디로 가는지는 앞단의 해시가 정한다. 뒤에 무엇이 붙어 있든 달라지지 않는다. 자기 이름만 돌려주는 껍데기 셋을 두고 방 2,000 개를 보내 어느 이름이 돌아오는지 읽었다.

**백엔드를 띄우면 재는 데 시간이 붙고 저장소와 Redis 가 값에 끼어들 자리만 생긴다.** 상황 11 과 20 에서 재는 자리를 틀린 것이 전부 재는 대상에 상관없는 것이 섞여 들어간 경우였다.

### 수치보다 어디로 갔는지가 중요했다

| 2→3 노드 | 옮겨 간 방 | 새 노드로 | 있던 노드끼리 |
|---|---|---|---|
| **consistent** | 602 / 2,000 (30.1%) | 602 | **0** |
| **그냥 hash** | 1,317 / 2,000 (65.8%) | 692 | **625** |

두 배 넘게 차이 나지만, 처음에 본 것은 그 배율이었다. 나눠 보니 다른 것이 보였다.

**`consistent` 는 옮겨 간 602 개가 전부 새 노드로 간 것이다.** 있던 노드끼리 자리를 바꾼 것이 한 건도 없다.

그냥 `hash` 는 1,317 개 중 **625 개가 있던 노드끼리 자리를 바꿨다.** 새 노드가 가져가는 692 개는 어느 방법을 써도 옮겨야 하는 몫이지만, 625 개는 그냥 끊긴 것이다. **끊을 이유가 없는 연결을 끊는다.**

배율만 봤으면 "두 배 낫다"로 적었을 것이다. 나눠 보니 **낫고 못한 정도가 아니라 하는 일이 다르다.**

### 덜 고른 것은 그대로 적는다

`consistent` 쪽이 노드별 방 수가 덜 고르다. 709 · 689 · 602 이고 그냥 `hash` 는 648 · 660 · 692 다. 링 위에 노드를 놓는 방식이라 구간이 정확히 삼등분되지 않는다.

**덜 고른 대신 옮겨 가는 것이 적다.** 이 서비스는 1:1 이라 방 하나가 두 명이고 709 대 602 는 문제가 되지 않는다. 단체 채팅이면 다시 봐야 한다.

### 반복 실행을 적지 않았다

해시는 같은 입력에 같은 답을 준다. 세 번 돌려 세 번 다 같은 값이었다. **재는 조건이 흔들리지 않으므로 여러 번 적을 이유가 없다.** 상황 19 나 24 처럼 시간을 재는 것과 다르다.

## 상황 26. 도구를 붙이자마자 없어도 되는 값을 무조건 꺼내는 자리가 나왔다

CI 는 테스트와 커버리지만 돌았다. **사람이 읽어서 넘어간 것을 보는 자리가 없었다.**

정적 분석을 붙였다. 커버리지와 같은 자리에 둔다. 보고서를 만들어 보여 주되 빌드를 깨지 않는다. 지금 나오는 것을 다 고친 뒤에 문을 거는 것이 순서고, **문부터 걸면 아무도 안 본다.**

### 22건 중 5건이 맞고 틀리고의 문제였다
...
STYLE 13건은 그대로 두었다. 취향 문제라 지금 고칠 이유가 없고, 보이는 자리에 남겨 두면 된다.

### 없어도 되게 만들어 놓고 무조건 꺼냈다

`createProduct` 는 카테고리와 시/도, 군/구, 동을 **모두 없어도 되게** 만들어 두었다. 요청에 없으면 `null` 로 두고 넘어간다. 요청 DTO 에는 검증 애너테이션이 하나도 없어 전부 선택이다.

그래 놓고 메서드 마지막 줄에서 넷을 전부 무조건 꺼냈다.

```java
sido.getName(), sido.getSidoId(), gungu.getName(), ...
```

**지역을 넣지 않고 상품을 만들면 상품과 파일을 다 저장한 뒤에 터진다.** 트랜잭션이라 되돌아가기는 하지만, 만들 수 있어야 하는 상품이 500 으로 막힌다.

검색도 같았다. `uploadType` 이 없으면 그대로 두도록 써 놓고 필터를 만들 때 무조건 꺼냈다. **타입을 고르지 않고 검색하면 터졌다.**

### 같은 요청에서 함께 비는 값을 마저 봤다

도구가 잡은 것을 고치다 보니 색인을 만드는 자리에도 같은 모양이 있었다. 종료일만 없을 수 있다고 다루고 **시작일과 거래 방법과 업로드 타입은 무조건 꺼내고 있었다.** 넷 다 같은 요청에서 함께 비는 값이다.

도구가 이것까지 잡지는 못했다. 메서드 경계를 넘어 엔티티를 지나가서다. **잡힌 것을 고치러 간 자리에서 눈으로 찾았다.**

### 도구를 바꾸면 뭔가 나온다

상황 15 에서 CI 를 붙이자마자 `gradlew` 실행 권한이 없는 것이 드러났다. 상황 23 에서 설정을 `yml` 로 옮기자마자 깨진 한글 주석 10 줄이 드러났다. 이번에도 붙이자마자 나왔다.

**세 번 다 "새로 만든 것"이 아니라 "원래 있었는데 아무도 안 보던 것"이다.**

### 제외 규칙에 무엇을 넣었나

Lombok 이 만든 접근자는 안에 있는 객체를 그대로 내주므로 `EI_EXPOSE_REP` 가 엔티티마다 붙는다. 우리가 쓴 코드가 아니고 고치려면 Lombok 을 걷어내야 한다. DTO 는 커버리지에서 뺀 것과 같은 이유로 뺐다.

**제외에 넣은 것은 "고치지 않기로 한 것"이 아니라 "이 도구가 이 코드에서는 틀리게 보는 것"이다.** 고칠 것을 여기 넣어 숨기면 도구를 붙인 뜻이 없어진다.

## 상황 27. 읽지도 못한 응답을 "돈이 안 옮겨졌다"고 단정했다

「아직 하지 않은 것」에 **`H0000`이 아닌 응답 코드를 나누지 않았다** 가 있다. 그 안에 다시 시도해야 할 것이 섞여 있을 수 있다고 적어 두었다.

나누려면 코드 표가 있어야 하는데 **붙을 API가 사라져 받아 둔 것이 없다.** 저장소 전체에서 아는 코드는 `H0000`(정상)과 `E4002`(이미 존재하는 ID) 둘뿐이다. **표를 지어내면 그것은 잰 것도 아는 것도 아니다.** 그래서 코드별 분류는 그대로 두었다.
...
if (!"H0000".equals(responseCode)) {
    return new TransferOutcome.Rejected(responseCode, ...);
}
```

`Header` 는 왔는데 `responseCode` 가 없으면 `String.valueOf` 가 **`"null"` 이라는 글자**를 만든다. 그 글자는 `H0000` 이 아니므로 거절이 된다. **읽지도 못한 응답이 거절 코드 `"null"` 을 달고 확정 실패가 됐다.**

`Rejected` 는 "돈은 옮겨지지 않은 것이 확정" 이라는 뜻이다. `TransferOutcome` 의 주석이 경계하는 바로 그것을, 코드를 못 읽은 자리에서 하고 있었다.

이제 코드가 없거나 비면 미확정으로 둔다. 미확정은 되돌리지 않고 남겨 두었다가 다시 물어 확정하는 자리(상황 22)가 이미 있다.

### 모르는 것을 아는 쪽으로 되돌리지 않는다

`fix/account-state-fallback` 에서 **모르는 계좌 상태 코드를 정상으로 되돌리지 않게** 고친 것과 같은 종류다. 그때는 모르는 것을 "정상"으로, 이번에는 못 읽은 것을 "거절"로 되돌리고 있었다.

**두 번 다 판정할 근거가 없는 자리에서 판정했다.** 값이 없을 때 무엇으로 채우는가는 기본값을 고르는 문제가 아니라 무엇을 주장하는가의 문제다.

### 나누지 못한 것은 나누지 못했다고 남긴다

코드별 분류는 여전히 하지 않았다. 「아직 하지 않은 것」에 **왜** 못 하는지를 더해 두었다. 나중에 다른 결제망을 붙일 때 표를 받으면 그때 나눈다.

## 상황 28. 배포를 처음 돌려 보니 여섯 군데서 걸렸다

상황 23 에서 배포를 Ansible 로 옮기고 **"플레이북 문법과 템플릿 렌더링까지 확인했다"** 고 적었다. 실제로 돌려 본 적은 없었다. 서버는 회수되어 사라졌다.

**서버가 없어도 돌려 볼 수 있다.** 대상을 이 기계로 잡으면 `git` 받기부터 건강 확인까지 그대로 지난다. 그렇게 했더니 여섯 군데서 걸렸다. **한 번도 성공할 수 없는 상태였다.**

### 1. 만드는 파일과 읽는 파일이 달랐다

배포는 `.env` 를 만드는데 `docker-compose.yml` 의 서비스 셋은 `env_file: .env.prod` 를 읽는다.

```
env file /home/ubuntu/joying/.env.prod not found
```

**세 번째 단계에서 죽는다.** Jenkins 를 쓰던 때 사람이 서버에 올려 두던 파일 이름이 그대로 남아 있었다. 상황 23 에서 그 파일을 없앤 것이 정확히 이것인데, 읽는 쪽을 같이 고치지 않았다.

### 2. 사람이 놓아 두어야 하는 파일이 아직 있었다

상황 23 에 **"서버에 사람이 미리 놓아 두어야 하는 파일이 없다"** 고 적었다. `ai/.env` 가 남아 있었다. AI 서비스가 그것을 읽고, 배포는 그것을 만들지 않는다.

그 안에 들어가야 하는 `GMS_API_KEY` 는 기본값이 없는 필수 값인데 **vault 예시에도 비밀 준비 스크립트에도 없었다.** 상황 23 에서 비밀 14 개를 세었는데 하나를 빠뜨린 것이다.

### 3. 건강을 확인하는 자리가 닿을 수 없었다

마지막 단계가 `http://localhost:8080/actuator/health` 를 30번 두드린다. **`backend` 서비스는 어디에도 포트를 열지 않는다.** 안쪽 네트워크에만 있다.

배포가 끝까지 가더라도 **마지막에서 늘 실패한다.** 루프백에만 연다. 밖으로 열면 앞단의 요청 제한을 지나지 않고 바로 닿는다.

### 4. 배포 위치가 두 군데에 적혀 있었다

`app_dir` 은 Ansible 이 정하는데 compose 에 `/home/ubuntu/joying/certbot/...` 이 글자로 박혀 있었다. **옮기면 조용히 깨진다.** compose 파일 기준 상대경로로 바꿨다.

### 5. 열쇠 한 줄이 앱 전체를 못 뜨게 했다

웹 푸시 VAPID 키에 아무 글자나 넣었더니 이렇게 죽었다.

```
Invalid point encoding 0x-53
  → pushService 빈 생성 실패
  → webPushService → chatService → chatRoomController
```

**푸시가 아니라 채팅이 같이 죽는다.** 설정은 `enabled=false` 와 빈 키는 다루고 있었지만 **틀린 키는 다루지 않았다.** 배포로 채워 넣는 값이라 눈으로 볼 기회도 없다.

푸시를 못 보내는 것과 아무것도 못 하는 것은 크기가 다르다. 이제 키가 없는 것과 같이 다룬다.

### 6. 커밋하지 않은 것은 배포되지 않는다

고쳐 놓고 돌렸는데 **같은 오류가 그대로 났다.** 배포는 `git` 으로 받으므로 커밋된 것만 간다. 두 번 겪고 알았다.

당연한 것인데, 손으로 배포하던 습관에서는 고친 파일이 바로 올라간다. 재는 자리를 틀린 것과 같은 종류다.

### 결과

```
TASK [떴는지 확인한다]  ok: [localhost]
TASK [결과를 적는다]    "배포 완료. 상태 확인 응답 200"
PLAY RECAP  ok=8  changed=5  failed=0
```

`{"status":"UP"}` 이 돌아온다. 저장소, 캐시, 검색, AI, 백엔드가 다 떴다.

### 앞단은 아직 뜨지 않는다

nginx 가 인증서를 찾지 못해 죽는다. 그 파일은 서버에만 있다. **서버를 새로 만들면 인증서를 받기 전에는 앞단이 뜨지 않고, 앞단이 떠야 certbot 이 인증을 받는다.** 서로를 기다린다.

이번에는 풀지 않았다. 리허설로 확인되는 자리가 아니어서가 아니라, **확인해 보니 드러난 것**이라 「아직 하지 않은 것」에 적는다.

### 도구를 붙이면 뭔가 나온다, 세 번째

상황 15 에서 CI 를 붙이자마자 `gradlew` 실행 권한이, 상황 23 에서 설정을 옮기자마자 깨진 주석이, 상황 26 에서 정적 분석을 붙이자마자 없어도 되는 값을 꺼내는 자리가 나왔다.

이번에는 **재 보지 않고 "준비됐다"고 적어 둔 것**이 나왔다. 네 번 다 새로 만든 것이 아니라 원래 있었는데 아무도 안 보던 것이다.

## 상황 29. 탭을 하나 더 열면 열에 일곱이 엉뚱한 노드로 간다

상황 25 에서 만든 자리로 두 가지를 더 쟀다.

### 한 노드가 죽는 것은 늘리고 줄이는 것과 다르다

늘리고 줄이는 것은 우리가 정해서 하는 일이고, 죽는 것은 아니다. 죽은 노드가 들고 있던 방은 어디론가 가야 하지만 **멀쩡한 노드의 방까지 흔들리면 안 된다.**

| 노드 하나가 죽었을 때 | 죽은 노드가 보던 방 | 멀쩡한 노드의 방 중 흔들린 것 |
|---|---|---|
| **consistent** | 195 (옮겨 갈 수밖에 없다) | **0** |
| **그냥 hash** | 214 (옮겨 갈 수밖에 없다) | **0** |

**둘이 같다.** nginx 는 죽은 서버를 목록에서 빼지 않는다. 목록은 그대로 두고 요청 단위로 다음 노드에 넘긴다. 노드 수가 바뀌지 않으므로 나머지 방의 해시도 바뀌지 않는다.

그래서 상황 25 에서 잰 `consistent` 의 이점은 **우리가 노드를 늘리거나 줄일 때**의 것이다. 죽는 것에는 해당하지 않는다. 그 문장을 좁혀 적는다.

### 탭을 둘 열면 나중 것이 앞선 방의 열쇠로 간다

앞단은 쿠키를 먼저 보고, 없을 때만 쿼리스트링을 본다. SockJS 가 쿼리스트링을 살려 주지 않아서 그렇게 만들었다(상황 21).

**쿠키는 브라우저에 하나다.** 방 A 를 보다가 탭을 열어 방 B 를 보면, B 로 가는 요청에 A 의 쿠키가 따라붙는다. 앞단은 쿠키를 먼저 보므로 **B 를 A 의 노드로 보낸다.**

방 500쌍을 보냈다. 노드 3개, `consistent`.

| | |
|---|---|
| 방 B 가 엉뚱한 노드로 간 쌍 | **345 / 499 (69.1%)** |

남은 30.9% 는 A 와 B 가 어차피 같은 노드였던 경우다. **노드가 셋이니 우연히 맞을 확률이 1/3 이고, 잰 값이 그것과 맞는다.**

이 상태에서 방 B 의 두 사람은 서로 다른 노드에 붙는다. 상황 21 에서 0 으로 만든 뒤집힘이 그 방에서는 그대로 돌아온다.

### 또 재는 자리를 틀렸다

처음에는 0% 가 나왔다. 아무 문제 없다는 뜻이었다.

**하네스에 열쇠 고르는 규칙을 빼먹었다.** 운영은 `map $cookie_chat_room $chat_room_key` 로 쿠키를 먼저 보는데, 하네스는 `$arg_roomId` 로만 해시하고 있었다. 그 상태에서는 쿠키를 무엇으로 넣든 값이 달라지지 않는다.

상황 21 에서 **부하 스크립트가 방 번호를 안 보내고 있어서** 0 이 안 나왔던 것과 같은 자리다. 그때는 열쇠가 빠져서 묶이지 않았고, 이번에는 열쇠 고르는 규칙이 빠져서 문제가 안 보였다.

**재는 쪽이 운영과 다르면 나오는 값은 운영의 값이 아니다.** 이번으로 재는 자리를 틀린 것이 다섯 번째다.

### 고치지 않았다

방마다 다른 열쇠를 쓰려면 쿠키 하나로는 안 된다. 연결 경로에 방 번호를 실어야 하는데, SockJS 가 쿼리스트링을 살려 주지 않아 쿠키로 돌아온 것이 상황 21 이다. 경로 자체를 바꾸거나(`/ws/chat/{roomId}`) SockJS 를 걷어내야 한다.

**둘 다 화면 쪽을 함께 고쳐야 해서 재는 것만 하고 남긴다.** 얼마나 나쁜지는 이제 안다.

## 상황 30. 첫 인증서를 아무도 만들지 않았다

상황 28 에서 배포를 끝까지 돌렸지만 **앞단은 뜨지 않았다.** 인증서를 못 찾아 죽는다고 적고 남겼다.

### 갱신만 하고 발급을 하지 않았다

compose 의 `certbot` 은 이렇게 돈다.

```
certbot renew --webroot -w /var/www/certbot --quiet
```

`renew` 는 **이미 있는 것을 갱신한다.** 없는 것을 만들지 않는다. 그래서 서버를 새로 만들면,

- 인증서가 없다 → 앞단이 죽는다
- 앞단이 죽어 있다 → `/.well-known/acme-challenge/` 를 아무도 응답하지 않는다
- 응답이 없다 → 인증을 받을 수 없다 → 인증서가 없다

**서로를 기다린다.** 이 서버는 사람이 손으로 한 번 받아 두었기 때문에 돌고 있었고, 그 사실이 어디에도 적혀 있지 않았다. 상황 23 에서 없앤 **"서버에 사람이 놓아 두어야 하는 것"** 이 하나 더 있었던 것이다.

### 자리를 채워 고리를 끊는다

배포가 인증서 자리를 보고, 비어 있으면 자체 서명한 것을 놓는다.

```
openssl req -x509 -newkey rsa:2048 -nodes -days 3650 -subj /CN={{ domain }}
```

**앞단이 뜬다.** 브라우저는 경고하지만 `/.well-known/acme-challenge/` 가 응답하므로 진짜 인증서를 받을 수 있다. 받은 것이 같은 자리에 덮어쓰고, 그 뒤로는 `renew` 가 할 일이 생긴다.

`creates:` 를 걸어 이미 있으면 건드리지 않는다. **두 번째 배포부터는 아무것도 하지 않는다.**
...
| | |
|---|---|
| `http://localhost/` | **301** (HTTPS 로 보낸다) |
| `https://localhost/` | **200** (화면이 뜬다) |
| `http://localhost:8080/actuator/health` | `{"status":"UP"}` |

저장소, 캐시, 검색, AI, 백엔드, 앞단이 전부 떴다. **상황 23 에서 "준비됐다"고 적은 뒤로 처음이다.**

### 도메인이 두 군데 적혀 있었다

`infra/nginx.conf` 의 `server_name` 과 인증서 경로에 도메인이 글자로 박혀 있다. 배포가 인증서를 놓을 자리를 알려면 같은 값이 필요해서 `vars.yml` 에 `domain` 을 두었다.

**지금은 두 군데가 같은 값을 들고 있다.** 상황 28 의 `app_dir` 과 같은 모양인데, nginx 설정은 이미지에 구워 넣으므로 배포 변수로 만들려면 템플릿으로 바꿔야 한다. 그것까지는 하지 않고 두 자리가 같아야 한다는 것만 주석에 적었다.

## 상황 31. 쿠키 하나로는 탭 둘을 구분할 수 없다

상황 29 에서 **탭을 둘 열면 69.1% 가 엉뚱한 노드로 간다**는 것을 쟀다. 고치려면 화면을 함께 손대야 해서 재는 것만 하고 남겼다. 이제 고친다.

### 왜 쿠키였나

앞단이 방 번호로 노드를 골라야 하는데, 그 값을 실어 보낼 자리가 마땅치 않았다. **SockJS 가 기본 주소 뒤에 `/{서버}/{세션}/websocket` 을 덧붙이므로 쿼리스트링이 살아남지 못한다.** `?roomId=5` 를 붙이면 `/ws/chat?roomId=5/123/abc/websocket` 이 된다.

그래서 쿠키로 알렸다. 쿠키는 모든 요청에 따라붙는다. **브라우저에 하나뿐이라는 것이 문제였다.**

### 경로는 앞쪽에 남는다

SockJS 가 덧붙이는 것은 **뒤쪽**이다. 기본 주소를 `/ws/chat/9001` 로 두면 `/ws/chat/9001/123/abc/websocket` 이 된다. 방 번호가 앞쪽에 그대로 있다.

### 뒤쪽에 엔드포인트를 더해 봤더니 404 였다

처음에는 뒤쪽이 그 주소를 받게 하려고 했다.

```java
registry.addEndpoint("/ws/chat", "/ws/chat/{roomId}")
```

`/ws/chat/9001/info` 가 **404** 였다. SockJS 핸들러가 `/ws/chat/**` 를 통째로 차지하고, 그 안에서 `9001/info` 를 `{서버}/{세션}` 으로 읽으려다 실패한다.

**뒤쪽을 건드리지 않기로 했다.** 앞단이 방 번호를 열쇠로 쓴 뒤 경로에서 걷어내고 넘긴다. 뒤쪽은 `/ws/chat` 을 그대로 받는다.

### `^~` 가 정규식 자리를 막고 있었다

방 번호가 실린 주소만 따로 받으려고 `location ~ ^/ws/chat/[0-9]+` 을 더했다. 여전히 404 였다.

**`location ^~ /ws/` 의 `^~` 는 정규식 위치를 아예 보지 않게 한다.** 그 접두사가 맞으면 거기서 끝난다. 새로 더한 자리는 한 번도 불리지 않았다.

기존 블록 안에서 걷어내는 것으로 바꿨다.

### `$uri` 는 걷어낸 뒤의 값이다

걷어내고 나니 이번에는 열쇠가 비었다. `map` 은 값을 **쓸 때** 계산되는데, 그 시점에는 `rewrite` 가 이미 `$uri` 를 바꿔 놓았다. 방 번호가 사라진 주소에서 방 번호를 찾고 있었다.

`$request_uri` 로 바꿨다. 그것은 고치기 전의 원래 주소다.

### 잰 것

방 500쌍, 노드 3개, `consistent`.

| 방 B 를 보는 탭에 방 A 의 쿠키가 남아 있을 때 | 엉뚱한 노드로 간 쌍 |
|---|---|
| **쿠키로 알릴 때** (옛 방식) | 316 / 499 (63.3%) |
| **경로에 실을 때** (지금) | **0 / 499 (0.0%)** |

옛 주소도 그대로 받는다. 배포 중에는 이미 붙어 있는 연결과 옛 화면이 섞인다. 앞단은 **경로 → 쿠키 → 쿼리스트링** 순서로 본다.

### 실제로 붙는지 확인했다

리허설로 전부 띄우고 SockJS 가 맨 처음 부르는 자리를 두드렸다.

```
GET /ws/chat/info        → 200   (옛 주소, 그대로 산다)
GET /ws/chat/9001/info   → 200   {"entropy":...,"websocket":true}
```

### 아직 하지 않은 것

**브라우저로 끝까지 붙여 보지는 않았다.** SockJS 가 맨 처음 부르는 자리까지 확인했고, 그 뒤 전송을 고르고 STOMP 를 얹는 것은 화면을 띄워야 볼 수 있다.

## 상황 32. 떴다고 채팅이 되는 것은 아니었다

상황 31 에서 방 번호를 연결 주소에 실었다. **SockJS 가 맨 처음 부르는 자리까지만 확인하고 남겼다.** 그 뒤로 전송을 고르고 STOMP 를 얹는 것은 화면을 띄워야 볼 수 있다고 적었다.

브라우저가 없어도 볼 수 있었다. **SockJS 의 `xhr-streaming` 전송은 그냥 HTTP 다.**

```
1) /ws/chat/{방}/info                              SockJS 가 이 주소에서 사는가
2) POST /ws/chat/{방}/{서버}/{세션}/xhr_streaming   세션이 열리는가 (o 가 온다)
3) POST .../xhr_send  with  CONNECT                STOMP CONNECTED 가 돌아오는가
```

### NUL 을 NUL 로 보내면 안 됐다

STOMP 프레임의 끝은 NUL 이다. 그대로 넣었더니 아무 답이 없었다. 오류도 없었다.

**SockJS 는 프레임을 JSON 배열에 담아 나른다.** JSON 문자열 안에 NUL 바이트를 그대로 넣으면 무효한 JSON 이고, 뒤쪽은 그것을 통째로 버린다. 여섯 글자짜리 `\u0000` 으로 적어야 한다.

조용히 버려지는 자리였다. `xhr_send` 는 **204** 를 돌려준다. 보낸 쪽만 보면 성공이다.

### 배포가 이것까지 본다

```
TASK [떴는지 확인한다]      ok    (actuator 200)
TASK [채팅이 붙는지 본다]   ok    (STOMP CONNECTED)
TASK [결과를 적는다]        "배포 완료. 상태 확인 응답 200, 채팅 연결 확인"
```

상황 28 에서는 건강 확인이 겨냥한 포트를 아무도 열지 않아 **늘 실패했다.** 이번은 반대다. **건강 확인이 통과해도 채팅은 안 붙을 수 있다.** 앞단이 방 번호를 걷어내는 자리가 어긋나면 화면에서만 조용히 안 된다.

`scripts/check-chat-handshake.sh` 로 따로도 돌릴 수 있다.

## 상황 33. 배포가 엉뚱한 프로세스를 보고 잘 떴다고 했다

상황 27 의 답장 실험을 재려는데 `/api/**` 가 전부 **404** 였다. 액추에이터는 200 이고 웹소켓도 붙는다. 스택을 통째로 내렸다 다시 올려도 같았다. **한 번은 같은 주소가 200 이었다.**

### 재는 자리가 아니라 부르는 자리가 틀렸다

```
$ lsof -nP -iTCP:8080 -sTCP:LISTEN
com.docke  40584  ...  IPv4  TCP 127.0.0.1:8080 (LISTEN)
java       41861  ...  IPv6  TCP *:8080 (LISTEN)
```

**둘이 떠 있었다.** 하나는 도커가 우리 백엔드를 묶어 둔 IPv4 자리, 하나는 이 기계에서 따로 돌던 다른 앱의 IPv6 자리다.

macOS 는 `localhost` 를 **IPv6 `::1` 로 먼저 푼다.** compose 는 백엔드를 `127.0.0.1:8080` 에, 즉 **IPv4 에만** 묶는다. 그래서 `localhost:8080` 으로 보낸 것은 전부 남의 앱으로 갔다.

남의 앱도 스프링 부트라 **액추에이터는 200 을 돌려주고** 우리 경로는 모르니 404 를 돌려준다. 겉으로는 "앱은 떠 있는데 컨트롤러만 사라진" 모습이 된다.

```
http://localhost:8080/api/v1/chat-rooms   404   (남의 앱)
http://127.0.0.1:8080/api/v1/chat-rooms   연결 거부  (우리 것은 그때 떠 있지도 않았다)
```

안에서 부르면 바로 드러났다. `joying-nginx` 에서 `joying-backend:8080` 으로 보내니 **연결 거부**였다. 우리 백엔드는 재시작을 되풀이하고 있었는데, 밖에서는 200 이 보여서 몰랐다.
...
**우리 것이 하나도 안 떠 있어도 남의 앱이 200 을 주면 배포는 성공이라고 적는다.** 실제로 그렇게 적혔다. `배포 완료. 상태 확인 응답 200` 을 받아 놓고, 그 순간 백엔드는 연결을 받지 못하고 있었다.

`127.0.0.1` 로 바꿨다. 도커가 묶은 그 자리다.

### 왜 앞단은 안 속았나

앞단 확인은 `https://localhost` 로 했는데 이것은 잘 돌았다. nginx 는 `0.0.0.0:443` 이라 IPv4 이고, IPv6 `::1:443` 에는 아무도 없다. **아무도 없으면 curl 이 다음 주소로 넘어간다.** 있으면 넘어가지 않는다.

**막힌 것이 아니라 답을 받은 것이 더 나빴다.** 거부당했으면 그 자리에서 알았다.

### 앞선 이상한 값들이 전부 설명된다

- k6 가 초당 13,556 요청에 실패 100% 를 낸 것 — 남의 앱이 404 를 즉시 돌려준 것이다
- 액추에이터가 `{"status":"DOWN"}` 을 준 것 — 남의 앱의 건강이다
- 메시지 500건을 넣고도 목록이 비어 있던 것 — 그 앱은 우리 데이터베이스를 모른다

**하나를 틀리니 그 뒤로 잰 것이 전부 남의 값이었다.** 상황 21 과 29 에서 재는 쪽에 열쇠가 빠져 값이 틀렸다면, 이번에는 재는 대상 자체가 남의 것이었다.

### 재는 자리를 여섯 번 틀렸다

상황 7 · 11 · 14 · 20 · 29 에 이어 여섯 번째다. 앞의 다섯은 **무엇을 재는가**가 틀렸고, 이번은 **누구를 재는가**가 틀렸다.

`localhost` 는 이름이지 주소가 아니다. 포트를 묶은 쪽과 부르는 쪽이 같은 계열을 쓰는지 확인하지 않으면, 이름은 조용히 다른 데로 간다.

### 고치고 나니 재려던 것이 재졌다

부르는 자리를 바로잡고 답장 실험을 마쳤다. 답장이 **각자 다른 것을 가리키면** 같은 하나를 가리킬 때보다 중앙값이 **25~40% 느리다**. 조회 횟수는 어느 쪽이나 한 번이고, 그 한 번이 가져오는 줄 수가 다르다. 수치는 [목록 조회 문서](../performance/message-list-nplus1.md#답장이-서로-다른-것을-가리키면)에 있다.

가는 길에 손으로 넣는 데이터의 함정 둘을 더 만났다. `is_deleted` 가 NULL 이면 목록이 **한 건도 안 나오고**, `is_edited` · `is_read` 가 NULL 이면 엔티티가 원시 `boolean` 이라 목록 조회가 **통째로 500** 이 된다. 앱이 쓸 때는 늘 채우므로 손으로 넣을 때만 생긴다. 시드 스크립트에 적어 두었다.
...
- **`H0000`이 아닌 응답 코드를 나누지 않았다.** 지금은 전부 확정 실패로 본다. 나누려면 코드 표가 있어야 하는데 붙을 API가 사라져 받아 둔 것이 없다. 아는 코드는 `H0000`과 `E4002` 둘뿐이다. 코드가 아예 오지 않은 경우만 상황 27 에서 갈랐다.
...
- **노드가 셋 이상일 때 메시지 순서를 재지 않았다.** 상황 29 에서 잰 것은 앞단이 어디로 보내는가까지다. 노드 셋에 실제로 붙여 뒤집힘을 세지는 않았다. 한 노드가 죽는 경우는 라우팅만 쟀다.
- **브라우저로는 붙여 보지 않았다.** 상황 32 에서 SockJS 의 xhr-streaming 전송으로 STOMP CONNECTED 까지 확인했다. 브라우저가 고르는 websocket 전송과 화면의 구독까지는 화면을 띄워야 볼 수 있다.
- **연결이 붙어 있는 중에 노드를 바꾸지는 않았다.** 상황 25 에서 잰 것은 방이 어디로 갈 것인가 하나다. 이미 붙어 있던 웹소켓이 그 순간 어떻게 끊기고 다시 붙는지는 재지 않았다.
- **정적 분석에 기준을 걸지 않았다.** 17 건이 남아 있고 보이게만 해 두었다. STYLE 13 건은 취향 문제라 그대로 둔다.
...
- **실서버에 배포하지 않았다.** 상황 28 에서 이 기계를 대상으로 끝까지 돌려 건강 확인 200 까지 봤다. 붙을 서버가 회수되어 사라졌다.
- **진짜 인증서를 받아 보지 않았다.** 상황 30 에서 자체 서명한 것으로 앞단을 띄웠다. Let's Encrypt 가 실제로 발급해 주는지는 도메인이 그 서버를 가리켜야 알 수 있다.
- **도메인이 두 군데 적혀 있다.** `infra/nginx.conf` 와 `vars.yml` 이 같은 값을 들고 있다. nginx 설정을 템플릿으로 바꿔야 한 군데로 모을 수 있다.
