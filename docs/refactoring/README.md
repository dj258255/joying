...
**드문 것이 아니라 열에 하나였다.** 같은 노드에서 0이 나오는 것이 원인을 못박는다.
...
**전부 한 노드로 몰린 것은 아닌가.** 몰리면 묶은 것이 아니라 한 대만 쓰는 것이다. 접근 로그에 업스트림 주소를 찍어 방 열 개를 보내니 6대 4로 갈렸다. 방 안에서는 묶이고 방끼리는 흩어진다.
...
방 단위 해시로 재는데 26~90회가 그대로 나왔다. 설정이 틀린 줄 알았다. **부하 스크립트가 방 번호를 안 보내고 있었다.** 이번에는 고친 쪽이 아니라 재는 쪽에 열쇠가 빠져 있었다.
...
- **결제 쪽 지표가 없다.** 결제 경합을 부하로 재서 고쳤지만 운영에서 볼 지표는 만들지 않았다.
