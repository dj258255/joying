...
/**
 * 반납 메시지 내용 파싱 (반납 전 영상 또는 반납 완료)
 * @param {string} content - 메시지 내용
 * @returns {Object|null} 파싱된 반납 정보
 */
export const parseReturnMessage = (content) => {
  ...
  // 반납 전 영상 메시지 감지
  if (content.includes('반납 전 영상을 촬영했습니다') || messageType === 'RETURN_VIDEO_UPLOADED') {
    return {
      type: 'VIDEO_UPLOADED',
      rentalHisId,
      messageType,
...
const ReturnMessageCard = ({ message, isOwn = false, onTrackClick, onReceiveConfirmClick }) => {
  ...
  if (!returnInfo) {
    // 반납 메시지가 아니면 null 반환 (기본 MessageBubble로 렌더링)
    return null;
  }

  // 반납 전 영상 메시지인 경우
  const isVideoMessage = returnInfo.type === 'VIDEO_UPLOADED';
  ...
  const courierName = returnInfo.courier ? (courierMap[returnInfo.courier] || returnInfo.courier) : null;
    ...
    <>
            ...
            <div className="bg-gradient-to-r from-gray-900 to-gray-800 text-white px-4 py-3 border-b border-gray-900/20">
              <div className="flex items-center gap-2">
                ...
                <p className="font-bold text-sm">
                  {isVideoMessage ? '반납 전 영상을 촬영했습니다' : '반납을 완료했습니다'}
                </p>
              </div>
            </div>
              ...
              {isVideoMessage ? (
                <>
                  <div className="flex items-center gap-2">
                    ...
                    <p className="text-sm text-gray-900 font-medium">반납 전 영상을 촬영했습니다</p>
                  </div>
                  {returnInfo.rentalHisId && (
                    <button
      ...
      {returnInfo.rentalHisId && (
        <VideoListModal
          ...
          rentalHisId={returnInfo.rentalHisId}
        />
      )}
    </>
  );
