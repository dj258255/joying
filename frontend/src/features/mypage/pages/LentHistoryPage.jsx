...
const LentHistoryPage = () => {
  ...
  const loadRentalHistory = () => {
    try {
      ...
      // 빌려준 내역에서 해당 rental 찾기
      const foundRental = DUMMY_RENTAL_HISTORY.lent.find(r => r.id === rentalId);
  ...
  const getReviews = () => {
    ...
    const myReview = DUMMY_REVIEWS.find(r => 
      ...
      r.reviewerId === 101 && 
      r.revieweeId === rental.renterId
    );
    
    // 상대가 작성한 리뷰 (빌렸을 때)
    const renterReview = DUMMY_REVIEWS.find(r => 
      r.productId === rental.productId && 
      r.reviewerId === rental.renterId && 
      r.revieweeId === 101
    );
    
    return { myReview, renterReview };
  };
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="p-6 max-w-6xl mx-auto">
        ...
        <div className="mb-6">
          ...
          <h1 className="text-2xl font-bold text-gray-900">빌려준 내역 상세</h1>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="bg-white rounded-2xl shadow-lg p-6">
            ...
            <div className="text-center mb-4">
              ...
              />
              <h3 className="text-lg font-semibold text-gray-900 mb-2">{rental.renter.username} 님 (Renter)</h3>
              <div className="flex items-center justify-center gap-2 mb-4">
                <div className="flex gap-1">
                  {renderPreciseStars(rental.renter.rating)}
                </div>
                <span className="text-sm text-gray-600">{rental.renter.rating} (124개 리뷰)</span>
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <button
                onClick={() => navigate(`/chats/${rental.renterId}`)}
                ...
                return (
                  <div>
                    ...
                    <div className="text-sm text-gray-500">
                      {renterReview.reviewer?.username} • {formatDate(renterReview.createdAt)}
                    </div>
                  </div>
                );
              } else {
