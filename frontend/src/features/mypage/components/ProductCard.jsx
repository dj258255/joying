      ...
      <div className="glass-product-image-main">
        {product?.thumbnailUrl ? (
          ...
          <div className="glass-product-image-placeholder-bg">
            <svg className="w-8 sm:w-12 h-8 sm:h-12 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        {(product?.dongId || product?.dong) && (
          <div className="absolute top-1.5 sm:top-3 left-1.5 sm:left-3 z-[5]">
            <span className="inline-flex items-center gap-0.5 sm:gap-1 px-1 sm:px-2 py-0.5 sm:py-1 bg-black/60 backdrop-blur-sm rounded text-[8px] sm:text-xs text-white/90">
              📍 {[product.sido, product.gugun, product.dong].filter(Boolean).join(' ')}
            </span>
          </div>
        )}
        ...
        {(onEdit || onDelete || (onAction && actionType === 'menu')) && (
          <div className="absolute top-1 sm:top-2 right-1 sm:right-2 z-[5]" ref={menuRef}>
            <button
              onClick={handleMenuClick}
              className="p-1 sm:p-1.5 bg-transparent rounded-lg hover:bg-white/10 transition-all duration-200 text-white"
              title="메뉴"
            >
            ...
            {/* 드롭다운 메뉴 */}
            {showMenu && (
              <div className="absolute top-full right-0 mt-1 sm:mt-2 w-24 sm:w-32 bg-white rounded-lg shadow-xl border border-gray-200 overflow-hidden z-50">
                {onEdit && (
                  <button
                    onClick={handleEditClick}
                    className="w-full px-2 sm:px-4 py-1.5 sm:py-2 text-left text-xs sm:text-sm text-gray-700 hover:bg-gray-100 transition-colors flex items-center gap-1 sm:gap-2"
                  >
                ...
                {onDelete && (
                  <button
                    ...
                    className="w-full px-2 sm:px-4 py-1.5 sm:py-2 text-left text-xs sm:text-sm text-red-600 hover:bg-red-50 transition-colors flex items-center gap-1 sm:gap-2"
                  >
                    <svg className="w-3 sm:w-4 h-3 sm:h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                    </svg>
                ...
                {!onEdit && !onDelete && onAction && actionType === 'menu' && (
                  <button
                    ...
                    className="w-full px-2 sm:px-4 py-1.5 sm:py-2 text-left text-xs sm:text-sm text-gray-700 hover:bg-gray-100 transition-colors"
                  >
                    액션
                  </button>
                )}
        ...
        {onAction && actionType !== 'menu' && !onEdit && !onDelete && (
          <div className="absolute top-1 sm:top-2 right-1 sm:right-2 z-[3] opacity-100">
            <button
              onClick={(e) => {
                e.stopPropagation();
                onAction();
              }}
              className={`p-1 sm:p-2 bg-transparent rounded-lg border-0 outline-none transition-all duration-200 hover:scale-110 flex items-center justify-center text-white`}
        ...
        {onLike && (
          <div className="absolute top-1 sm:top-2 right-1 sm:right-2 z-[3] opacity-100">
            <button
              ...
              className="p-1 sm:p-2 bg-transparent rounded-lg border-0 outline-none transition-all duration-200 hover:scale-110 flex items-center justify-center"
        ...
        <div className="glass-product-text-overlay">
          {/* 제품명 */}
          <h3 className="text-[10px] sm:text-sm font-bold text-white mb-1 sm:mb-2 line-clamp-1">
